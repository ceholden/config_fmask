...
def plcloud(filename, cldprob=22.5, num_Lst=None, images=None, log_filename="FMASK_LOGFILE.txt",
    ...
    # Change to Temp's size so we can operate on subsets
    Cloud = numpy.zeros(Temp.shape,'uint8') # cloud mask
    Snow  = numpy.zeros(Temp.shape,'uint8') # Snow mask
    WT    = numpy.zeros(Temp.shape,'uint8') # Water msk
